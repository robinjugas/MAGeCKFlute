...
#'
# ScatterView_v_1.1
ScatterView <-  function (data, x = "x", y = "y", label = 0, model = c("none", 
                                                                       "ninesquare", "volcano", "rank")[1], x_cut = NULL, y_cut = NULL, 
                          ...
                          legend.position = "None", ...) 
{
  ...
  mycolour = c("#1f78b4", "#fb8072", "#33a02c", "#ff7f00", 
               "#EB7B63", "#66c2a5", "#0000ff", "#fdb462", "#ffed6f", 
  ...
  if (color == "group") {
    ...
    if (mode(gg[, color]) == "numeric") 
      p = p + scale_color_gradient2(low = "#377eb8", high = "#e41a1c", 
                                    midpoint = 0)
    else if (!"try-error" %in% class(try(col2rgb(gg[1, color]), 
                                         silent = TRUE))) {
